...
namespace Microsoft.VisualStudio.TestTools.UnitTesting
{
    ...
    public static partial class AssertEx
    {
        ...
        public static void Is(string actual, string expected, string message = "")
        {
            var result = actual.StringDiffOnly(expected);
            int cnt = result.Where(x => x.Value == WILDCARD).Count();
            Assert.AreEqual(result.Count, cnt, message);
        }
        ...
        public static void IsContains<T>(this T value, IEnumerable<T> expected, string message = "")
        {
            expected.Contains<T>(value).Is(true, message);
        }
        ...
        static EqualInfo StructuralEqual(object left, object right, IEnumerable<string> names)
        {
            ...
            if (Type.GetTypeCode(type) != TypeCode.Object)
            {
                ...
                {
                    var result = left.ToString().StringDiffOnly(right.ToString());
                    int cnt = result.Where(x => x.Value == WILDCARD).Count();
                    return new EqualInfo { IsEquals = (result.Count == cnt), Left = left, Right = right, Names = names };
                }
                else
                {
            ...
            // IEquatable<T>
            /* Equalsを実装していなくてもIsAssignableFromがtrueになるため一旦コメントアウト */
            /*
