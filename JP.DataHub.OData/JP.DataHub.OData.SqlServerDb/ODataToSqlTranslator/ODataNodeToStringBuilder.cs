        ...
        private string TranslateFunctionCall(string functionName, IEnumerable<QueryNode> argumentNodes)
        {
            ...
            switch (functionName)
            {
                case Constants.KeywordSubstring:
                    // SQLServerのSUBSTRINGは1スタートのため開始位置を修正
                    ...
                    // substringの第三引数を補完
                    if (functionParams.Count() == 2)
                    {
                        functionParams.Add(int.MaxValue.ToString());
                    }
                ...
                case Constants.KeywordIndexOf:
                    // 他のデータソースに合わせて0基準に修正
                    var x = string.Concat(QueryFormatter.TranslateFunctionName(functionName), Constants.SymbolOpenParen, TranslateNode(list[1]), Constants.SymbolComma, TranslateNode(list[0]), Constants.SymbolClosedParen);
                    return $"({x} - 1)";
