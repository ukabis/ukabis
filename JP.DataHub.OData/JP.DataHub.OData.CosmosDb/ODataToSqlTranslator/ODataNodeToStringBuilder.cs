...
namespace JP.DataHub.OData.CosmosDb.ODataToSqlTranslator
{
    ...
    internal class ODataNodeToStringBuilder : QueryNodeVisitor<string>
    {
        ...
        public override string Visit(AnyNode node)
        {
            ...
            return string.Concat(Constants.Delimiter, source, Constants.Delimiter, Constants.SymbolOpenParen, body, Constants.SymbolClosedParen);
            //}
        }
        ...
        private string TranslateFunctionCall(string functionName, IEnumerable<QueryNode> argumentNodes)
        {
            ...
            // substringの第三引数を補完
            if (functionName == Constants.KeywordSubstring && argumentNodes.Count() == 2)
            {
