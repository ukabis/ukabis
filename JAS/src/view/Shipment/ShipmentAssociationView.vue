...
const { associationCheckListObjs } = storeToRefs(useData)

const check = (index: number) => {
    associationCheckListObjs.value[index].value = !associationCheckListObjs.value[index].value
}

const checkedValues = ref<boolean[]>([])

const clickBlackBtn = () => {
    useUtil.transition("/shipment/association_confirm")
}
...
<template>
    <Header title-label="JAS・センサー紐付け登録" line-type="shipment" />
    <div class="bg">
        <div class="content" ref="contentRef">
            ...
            <LeftLabelSelectBox name="group" label="流通行程管理グループ" :option="groupListOpt" />
            <div class="check_box_wrap">
                <ul class="check_box_items">
                    <li class="check_box_item" v-for="obj, index in associationCheckListObjs">
                        <span class="checkbox">
