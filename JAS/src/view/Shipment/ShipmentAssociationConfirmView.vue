<!-- ShipmentAssociationConfirmView -->
...
const isCheckListObjsTrueItemExist = () => {
    let trueItemNum = 0
    associationCheckListObjs.value.map(item => {
        if (item.value) {
            trueItemNum++
        }
    })
...
const back = () => {
    useUtil.transition("/shipment/association")
}
        ...
        <div class="content" ref="contentRef">
            ...
            <div class="check_items_wrap">
                <ul class="check_items" v-if="isCheckListObjsTrueItemExist()">
                    <li class="check_item" :class="{ hidden: !item.value }" v-for="item in associationCheckListObjs">{{ item.label }}</li>
                </ul>
            ...
            <div class="btn_group">
                ...
                <BlackBgBtn btn-label="送信する" input-width="auto" />
            </div>
        </div>
    </div>
    <Footer />
