...
{
    [TestClass]
    public class UnitTest_SqlServerDataStoreRepository : UnitTestBase
    {
        private const string RepositoryName = "SqlServer";
        ...
        public override void TestInitialize()
        {
            ...
            UnityContainer.RegisterType<INewDynamicApiDataStoreRdbmsRepository, SqlServerDataStoreRepository>();
        ...
        public void QueryOnce_OK_バージョン情報()
        {
            ...
            mockQuerySqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
            ...
            target.RepositoryInfo = new RepositoryInfo(RepositoryType.SQLServer2.ToCode(), new Dictionary<string, bool>() { { ConnectionString, false } });
...
{{ 
    ...
    ""_etag"": 1
            ...
            UnityContainer.RegisterInstance<IJPDataHubRdbms>("SqlServer", mockDb.Object);
...
{{ 
    ""id"": ""hoge"",
    ""documentversions"": ""[{{ 'key1': 'value1' }},{{ 'key2': 'value2' }}]"",
    ""DocumentIdForAttachFile"": ""foobar"",
    ""_Type"": ""fuga"",
    ""_Reguser_Id"": ""piyo1"",
    ""_Regdate"": ""2011-12-23T11:12:23"",
    ""_Upduser_Id"": ""piyo2"",
    ""_Upddate"": ""2021-12-23T11:12:23"",
    ""_etag"": 1
}}") });
            UnityContainer.RegisterInstance<IJPDataHubRdbms>("SqlServer", mockDb.Object);
            ...
            // 対象メソッド実行
            var queryParam = ValueObjectUtil.Create<QueryParam>(dummyAction, new OperationInfo(OperationInfo.OperationType.DocumentVersion), new NativeQuery(Guid.NewGuid().ToString(), new Dictionary<string, object>(), true));
            var result = target.QueryOnce(queryParam);
            result.Value.ToString().Is(JToken.Parse($@"
{{ 
    ""id"": ""hoge"",
    ...
    ""_etag"": 1
        ...
        [TestMethod]
        public void QueryOnce_NG_SqlException_207_SqlServerTransientExceptionDetector_miss()
        {
            var pagingInfo = new PagingInfo() { IsPaging = false, OffsetCount = null, FetchCount = null };
            ...
            UnityContainer.RegisterInstance("SqlServer", mockDb.Object);
            ...
            // SqlServerTransientExceptionDetectorに含まれないのでretryはされない
        ...
        [TestMethod]
        public void QueryOnce_NG_SqlException_SqlServerTransientExceptionDetector_hit()
        {
            var pagingInfo = new PagingInfo() { IsPaging = false, OffsetCount = null, FetchCount = null };
            ...
            // SqlServerTransientExceptionDetectorに含まれるのでretry
            mockDb.Verify(x => x.QueryDocument(
                    It.Is<string>(y => y != "COUNT"),
                    It.IsAny<object>()),
                Times.Exactly(3));
        ...
        [TestMethod]
        public void Query_NG_SqlException_207_SqlServerTransientExceptionDetector_miss()
        {
            var pagingInfo = new PagingInfo() { IsPaging = false, OffsetCount = null, FetchCount = null };
            var mockQuerySqlBuilder = new Mock<IQuerySqlBuilder>();
            mockQuerySqlBuilder.SetupGet(x => x.PagingInfo).Returns(pagingInfo);
        ...
        [TestMethod]
        public void Query_NG_SqlException_SqlServerTransientExceptionDetector_hit()
        {
            var pagingInfo = new PagingInfo() { IsPaging = false, OffsetCount = null, FetchCount = null };
            var mockQuerySqlBuilder = new Mock<IQuerySqlBuilder>();
            mockQuerySqlBuilder.SetupGet(x => x.PagingInfo).Returns(pagingInfo);
        ...
        [TestMethod]
        public void QueryEnumerable_NG_SqlException_SqlServerTransientExceptionDetector_miss()
        {
            var pagingInfo = new PagingInfo() { IsPaging = false, OffsetCount = null, FetchCount = null };
            var mockQuerySqlBuilder = new Mock<IQuerySqlBuilder>();
            mockQuerySqlBuilder.SetupGet(x => x.PagingInfo).Returns(pagingInfo);
        ...
        [TestMethod]
        public void QueryEnumerable_NG_SqlException_SqlServerTransientExceptionDetector_hit()
        {
            var pagingInfo = new PagingInfo() { IsPaging = false, OffsetCount = null, FetchCount = null };
            var mockQuerySqlBuilder = new Mock<IQuerySqlBuilder>();
            mockQuerySqlBuilder.SetupGet(x => x.PagingInfo).Returns(pagingInfo);
        ...
        private (IEnumerable<JToken>, IEnumerable<JsonDocument>) CreateQueryTestData(int expectCount)
        {
            var targetData = Enumerable.Range(1, expectCount)
                .Select(x => (JToken)new JObject { ["id"] = $"id{x}", ["stringvalue"] = $"string{x}", ["intvalue"] = x, ["objectvalue"] = JToken.Parse($"{{ 'key{x}': 'value{x}' }}").ToString(), ["arrayvalue"] = JToken.Parse($"[ 'value{x}-1', 'value{x}-2']").ToString(), ["truevalue"] = true, ["falsevalue"] = false, ["_etag"] = 2 })
                .ToList();

            var expectData = Enumerable.Range(1, expectCount)
                .Select(x => new JsonDocument(new JObject { ["id"] = $"id{x}", ["stringvalue"] = $"string{x}", ["intvalue"] = x, ["objectvalue"] = JToken.Parse($"{{ 'key{x}': 'value{x}' }}"), ["arrayvalue"] = JToken.Parse($"[ 'value{x}-1', 'value{x}-2']"), ["truevalue"] = true, ["falsevalue"] = false, ["_etag"] = "2" }))
                .ToList();
...
{{
    ...
    'key': 'value'
        ...
        [TestMethod]
        public void RegisterOnce_WithAdditionalSqls()
        {
            (var target, var mockDb) = CreateDummyRegisterRepository(true);
            var mockQuerySqlBuilder = new Mock<IUpsertSqlBuilder>();
            mockQuerySqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
            mockQuerySqlBuilder.SetupGet(x => x.HasPostAdditionalSqls).Returns(true);
            mockQuerySqlBuilder.SetupGet(x => x.PostAdditionalSqls).Returns(new List<(string, IRdbmsSqlParameterList)>() 
            { 
                ("", new SqlServerSqlParameterList()),
                ("", new SqlServerSqlParameterList())
            });
...
{{
    ...
    '_Version': 1
        ...
        [TestMethod]
        public void RegisterOnce_楽観排他NG__SqlServerTransientExceptionDetector_miss()
        {
...
{{
    ...
    'key': 'value'
        ...
        [TestMethod]
        public void RegisterOnce_NG_SqlException_SqlServerTransientExceptionDetector_hit()
        {
            var mockQuerySqlBuilder = new Mock<IUpsertSqlBuilder>();
            mockQuerySqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
            UnityContainer.RegisterInstance<IUpsertSqlBuilder>(RepositoryName, mockQuerySqlBuilder.Object);
...
{{
    ...
    'key': 'value'
        ...
        [TestMethod]
        public void DeleteOnce_WithAdditionalSqls()
        {
            (var target, var mockDb) = CreateDummyDeleteRepository();
            var mockDeleteSqlBuilder = new Mock<IDeleteSqlBuilder>();
            mockDeleteSqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
            mockDeleteSqlBuilder.SetupGet(x => x.HasPreAdditionalSqls).Returns(true);
            mockDeleteSqlBuilder.SetupGet(x => x.PreAdditionalSqls).Returns(new List<(string, IRdbmsSqlParameterList)>()
            {
                ("", new SqlServerSqlParameterList()),
                ("", new SqlServerSqlParameterList())
            });
...
{{
    ...
    '_etag': '1'
            ...
            {
                callbackCalled = true;
                y.Is(RepositoryType.SQLServer2);
...
{{
    ...
    'key': 'value'
        ...
        [TestMethod]
        public void DeleteOnce_NG_SqlException_SqlServerTransientExceptionDetector_hit()
        {
            var mockDeleteSqlBuilder = new Mock<IDeleteSqlBuilder>();
            mockDeleteSqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
            UnityContainer.RegisterInstance<IDeleteSqlBuilder>(RepositoryName, mockDeleteSqlBuilder.Object);
...
{{
    ...
    'key2': 'newvalue2'
        ...
        [TestMethod]
        public void ODataPatch_NG_SqlException_207_SqlServerTransientExceptionDetector_miss()
        {
            var expectedAffectedCount = 10;
            var mockSqlBuilder = new Mock<IODataPatchSqlBuilder>();
            mockSqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
...
{{
    ...
    'key2': 'newvalue2'
        ...
        [TestMethod]
        public void ODataPatch_NG_SqlException_SqlServerTransientExceptionDetector_hit()
        {
            var expectedAffectedCount = 10;
            var mockSqlBuilder = new Mock<IODataPatchSqlBuilder>();
            mockSqlBuilder.SetupGet(x => x.SqlParameterList).Returns(new SqlServerSqlParameterList());
