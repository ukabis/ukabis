...
{
    [TestClass]
    public class UnitTest_SqlServerUpsertSqlBuilder : UnitTestBase
    {
        ...
        public override void TestInitialize()
        {
            ...
            UnityContainer.RegisterType<IUpsertSqlBuilder, SqlServerUpsertSqlBuilder>(RepositoryType.SQLServer2.ToCode(), new Interceptor<InterfaceInterceptor>(), new InterceptionBehavior<PolicyInjectionBehavior>(),
...
USING (
    SELECT @p_1 AS ""p_1"", @p_2 AS ""p_2"", @p_3 AS ""p_3"", @p_4 AS ""p_4"", @p_5 AS ""p_5"", @p_6 AS ""p_6"", @p_7 AS ""p_7"", @p_8 AS ""p_8"", @p_9 AS ""p_9"", @p_10 AS ""p_10"", @p_11 AS ""p_11"", @p_12 AS ""p_12"", @p_13 AS ""p_13"", @p_14 AS ""p_14"", @p_15 AS ""p_15""
) X ON (""{_tableName}"".""id"" = @p_2)
WHEN MATCHED AND ""{_tableName}"".""_etag"" = @p_16 THEN
...
{{ 
    ...
    '_etag': '5'
            ...
            builder.Sql.Is($@"MERGE INTO ""{_tableName}""
USING (
    SELECT @p_1 AS ""p_1"", @p_2 AS ""p_2"", @p_3 AS ""p_3"", @p_4 AS ""p_4"", @p_5 AS ""p_5"", @p_6 AS ""p_6"", @p_7 AS ""p_7"", @p_8 AS ""p_8""
) X ON (""{_tableName}"".""id"" = @p_2)
WHEN MATCHED THEN
...
WHEN MATCHED AND ""{versionTableName}"".""_etag"" = @p_9 THEN
...
{{ 
    ...
    'FileName': 'fuga'
            ...
            var repositoryName = RepositoryType.SQLServer2.ToCode();
            var registerParam = ValueObjectUtil.Create<RegisterParam>(json, GetDataSchema(false), operationInfo, repositoryKey);
            var repositoryInfo = new RepositoryInfo(RepositoryType.SQLServer2.ToCode(), new Dictionary<string, bool>() { { Guid.NewGuid().ToString(), false } });
            var mockContainerDynamicSeparation = new Mock<IContainerDynamicSeparationRepository>();
            mockContainerDynamicSeparation
                .Setup(x => x.GetOrRegisterContainerName(It.IsAny<PhysicalRepositoryId>(), It.IsAny<ControllerId>(), It.IsAny<VendorId>(), It.IsAny<SystemId>(), It.IsAny<OpenId>()))
            ...
            builder.BuildUp();
            builder.Sql.Is($@"MERGE INTO ""AttachFileMeta""
USING (
    SELECT @p_1 AS ""p_1"", @p_2 AS ""p_2"", @p_3 AS ""p_3"", @p_4 AS ""p_4"", @p_5 AS ""p_5""
) X ON (""AttachFileMeta"".""id"" = @p_2)
WHEN MATCHED THEN
UPDATE SET ""FileId"" = X.""p_3"", ""FileName"" = X.""p_4"", ""_Type"" = X.""p_5"", ""_etag"" = ""_etag"" + 1
WHEN NOT MATCHED THEN
INSERT (""_etag"", ""id"", ""FileId"", ""FileName"", ""_Type"") VALUES (X.""p_1"", X.""p_2"", X.""p_3"", X.""p_4"", X.""p_5"")
;
");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
            ...
            builder.HasPostAdditionalSqls.IsTrue();
            builder.PostAdditionalSqls.Count().Is(1);
            builder.PostAdditionalSqls.First().Sql.Is(@"
INSERT INTO AttachFileMetaSearch
(
    id,
    AttachFileMetaId,
    MetaKey,
...
SELECT
    ...
    _Upduser_Id,
    _Upddate
FROM AttachFileMeta CROSS APPLY OPENJSON(MetaList) WITH
(
    MetaKey   NVARCHAR(400) '$.MetaKey',
    MetaValue NVARCHAR(400) '$.MetaValue'
)
WHERE FileId = @FileId");
            builder.PostAdditionalSqls.First().SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { "@FileId", "hoge" }
            });
        }
...
{{ 
    ...
    'DocumentIdForAttachFile': 'hoge',
    '_Reguser_Id': 'piyo1',
    '_Regdate': '2001-12-23T01:12-23',
    '_Upduser_Id': 'piyo2',
    '_Upddate': '2011-12-23T01:12-23'
}}");
            var expectedDocumentVersions = JToken.Parse($@"[ {{ 'key1': 'value1' }}, {{ 'key2': 'value2' }}]");

            var operationInfo = new OperationInfo(OperationInfo.OperationType.DocumentVersion);
            var repositoryKey = new RepositoryKey("API~piyo");
            var repositoryName = RepositoryType.SQLServer2.ToCode();
            var registerParam = ValueObjectUtil.Create<RegisterParam>(json, GetDataSchema(false), operationInfo, repositoryKey);
            var repositoryInfo = new RepositoryInfo(RepositoryType.SQLServer2.ToCode(), new Dictionary<string, bool>() { { Guid.NewGuid().ToString(), false } });
            ...
            builder.BuildUp();
            builder.Sql.Is($@"MERGE INTO ""DocumentVersion""
USING (
    SELECT @p_1 AS ""p_1"", @p_2 AS ""p_2"", @p_3 AS ""p_3"", @p_4 AS ""p_4"", @p_5 AS ""p_5"", @p_6 AS ""p_6"", @p_7 AS ""p_7"", @p_8 AS ""p_8"", @p_9 AS ""p_9""
) X ON (""DocumentVersion"".""id"" = @p_2)
WHEN MATCHED THEN
UPDATE SET ""documentversions"" = X.""p_3"", ""DocumentIdForAttachFile"" = X.""p_4"", ""_Reguser_Id"" = X.""p_5"", ""_Regdate"" = X.""p_6"", ""_Upduser_Id"" = X.""p_7"", ""_Upddate"" = X.""p_8"", ""_Type"" = X.""p_9"", ""_etag"" = ""_etag"" + 1
WHEN NOT MATCHED THEN
INSERT (""_etag"", ""id"", ""documentversions"", ""DocumentIdForAttachFile"", ""_Reguser_Id"", ""_Regdate"", ""_Upduser_Id"", ""_Upddate"", ""_Type"") VALUES (X.""p_1"", X.""p_2"", X.""p_3"", X.""p_4"", X.""p_5"", X.""p_6"", X.""p_7"", X.""p_8"", X.""p_9"")
;
");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { "@p_1", 1L },
                { "@p_2", id },
                { "@p_3", expectedDocumentVersions.ToString() },
                { "@p_4", "hoge" },
                { "@p_5", "piyo1" },
                { "@p_6", "2001-12-23T01:12-23" },
                { "@p_7", "piyo2" },
                { "@p_8", "2011-12-23T01:12-23" },
                { "@p_9", "API~piyo" }
            });

            builder.HasPostAdditionalSqls.IsFalse();
        }
