...
{
    [TestClass]
    public class UnitTest_SqlServerODataPatchSqlBuilder : UnitTestBase
    {
        ...
        public override void TestInitialize()
        {
            ...
            UnityContainer.RegisterType<IODataPatchSqlBuilder, SqlServerODataPatchSqlBuilder>(RepositoryType.SQLServer2.ToCode(), new Interceptor<InterfaceInterceptor>(), new InterceptionBehavior<PolicyInjectionBehavior>(),
        ...
        public void BuildUp_追加条件なし()
        {
            ...
            var nativeQuery = new NativeQuery($"SELECT * FROM {{TABLE_NAME}} WHERE {odataFilterCondition}", odataFilterParameters);
...
{{ 
    ...
    '_Where': {{
        ...
        'Object': [ 'AA', 'BB', 'CC' ]
                ...
                builder.Sql.Is($@"UPDATE ""{_tableName}""
SET ""STR_VALUE"" = @odp_1, ""STR_NULL"" = @odp_2, ""INT_VALUE"" = @odp_3, ""DBL_VALUE"" = @odp_4, ""NUM_NULL"" = @odp_5, ""OBJ_VALUE"" = @odp_6, ""OBJ_NULL"" = @odp_7, ""ARY_VALUE"" = @odp_8, ""ARY_NULL"" = @odp_9, ""BOL_VALUE"" = @odp_10, ""BOL_NULL"" = @odp_11, ""DAT_VALUE"" = @odp_12, ""DAT_NULL"" = @odp_13, ""_etag"" = ""_etag"" + 1
WHERE {odataFilterCondition} AND ""{columnName}"" in (@odpex_1,@odpex_2,@odpex_3)
");
        ...
        public void BuildUp_追加条件あり_条件不正()
        {
            TestContext.Run((string columnName, string @operator, string @object) =>
            {
                ...
                var nativeQuery = new NativeQuery($"SELECT * FROM {{TABLE_NAME}} WHERE {Guid.NewGuid()}", new Dictionary<string, object>());
                var patchData = JToken.Parse($@"
{{ 
    'STR_VALUE': 'abc',
    '_Where': {{
