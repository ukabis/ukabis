...
{
    [TestClass]
    public class UnitTest_SqlServerDeleteSqlBuilder : UnitTestBase
    {
        ...
        public override void TestInitialize()
        {
            ...
            UnityContainer.RegisterType<IDeleteSqlBuilder, SqlServerDeleteSqlBuilder>(RepositoryType.SQLServer2.ToCode(), new Interceptor<InterfaceInterceptor>(), new InterceptionBehavior<PolicyInjectionBehavior>(),
        ...
        public void BuildUp_添付ファイルメタ()
        {
            ...
            var repositoryName = RepositoryType.SQLServer2.ToCode();
            var deleteParam = ValueObjectUtil.Create<DeleteParam>(json, operationInfo);
            var repositoryInfo = new RepositoryInfo(RepositoryType.SQLServer2.ToCode(), new Dictionary<string, bool>() { { Guid.NewGuid().ToString(), false } });
            var mockContainerDynamicSeparation = new Mock<IContainerDynamicSeparationRepository>();
            mockContainerDynamicSeparation
                .Setup(x => x.GetOrRegisterContainerName(It.IsAny<PhysicalRepositoryId>(), It.IsAny<ControllerId>(), It.IsAny<VendorId>(), It.IsAny<SystemId>(), It.IsAny<OpenId>()))
            ...
            builder.BuildUp();
            builder.Sql.Is($"DELETE FROM \"AttachFileMeta\" WHERE \"id\" = @p_1");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { "@p_1", id }
            });
            ...
            // 追加SQL
            builder.HasPreAdditionalSqls.IsTrue();
            builder.PreAdditionalSqls.Count().Is(1);
            builder.PreAdditionalSqls.First().Sql.Is("DELETE FROM AttachFileMetaSearch WHERE AttachFileMetaId = @AttachFileMetaId");
            builder.PreAdditionalSqls.First().SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { "@AttachFileMetaId", id }
            });
        }
