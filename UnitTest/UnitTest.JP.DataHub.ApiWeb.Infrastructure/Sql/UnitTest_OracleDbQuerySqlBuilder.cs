...
{
    [TestClass]
    public class UnitTest_OracleDbQuerySqlBuilder : UnitTestBase
    {
        ...
        public override void TestInitialize()
        {
            ...
            UnityContainer.RegisterType<IQuerySqlBuilder, OracleDbQuerySqlBuilder>(RepositoryType.OracleDb.ToCode(), new Interceptor<InterfaceInterceptor>(), new InterceptionBehavior<PolicyInjectionBehavior>(),
        ...
        public void BuildUp_Query_レスポンスモデルあり_リソースモデルと同じ()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 }
            });
        ...
        public void BuildUp_Query_レスポンスモデルあり_リソースモデルと異なる_additionalProperties_false()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"id\",\"_Owner_Id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 }
            });
        ...
        public void BuildUp_Query_楽観排他()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\",\"_etag\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 }
            });
        ...
        public void BuildUp_Query_ベンダー依存()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1 AND \"_Vendor_Id\" = :p_2 AND \"_System_Id\" = :p_3");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 },
                { ":p_2", vendorId.Value },
        ...
        public void BuildUp_Query_個人依存()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1 AND \"_Owner_Id\" = :p_2");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 },
                { ":p_2", openId.Value }
        ...
        public void BuildUp_Query_管理項目取得_依存なし()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\",\"_Version\",\"_Reguser_Id\",\"_Regdate\",\"_Upduser_Id\",\"_Upddate\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 }
            });
        ...
        public void BuildUp_Query_管理項目取得_ベンダー依存()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\",\"_Version\",\"_Reguser_Id\",\"_Regdate\",\"_Upduser_Id\",\"_Upddate\",\"_Vendor_Id\",\"_System_Id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1 AND \"_Vendor_Id\" = :p_2 AND \"_System_Id\" = :p_3");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 },
                { ":p_2", vendorId.Value },
        ...
        public void BuildUp_Query_QueryStringあり()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"hoge\",\"fuga\",\"foo\",\"bar\",\"id\",\"_Owner_Id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1 AND \"hoge\" = :p_2 AND \"fuga\" = :p_3");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 },
                { ":p_2", stringValue },
        ...
        public void BuildUp_Query_APIクエリあり_JOINあり()
        {
            ...
            mockDynamicApiRepository
                .Setup(x => x.GetPhysicalRepositoryIdByControllerId(It.IsAny<ControllerId>(), It.Is<RepositoryType>(y => y == RepositoryType.OracleDb)))
            ...
            mockDynamicApiRepository.Verify(x => x.GetPhysicalRepositoryIdByControllerId(It.IsAny<ControllerId>(), It.Is<RepositoryType>(y => y == RepositoryType.OracleDb)), Times.Exactly(1));
        }
        ...
        public void BuildUp_Delete_APIクエリなし()
        {
            ...
            var builder = CreateBuilder(queryParam);
            builder.BuildUp();
            builder.Sql.Is($"SELECT \"id\" FROM \"{_tableName}\" WHERE \"_Version\" = :p_1");
            builder.SqlParameterList.ToDictionary(x => x.Value.SqlParameter, y => y.Value.ParameterValue).IsStructuralEqual(new Dictionary<string, object>()
            {
                { ":p_1", 5 }
            });
        ...
        public void OrderByPattern()
        {
            ...
            {
                new Regex(OracleDbQuerySqlBuilder.OrderByPattern, RegexOptions.IgnoreCase).IsMatch(target).Is(expected);
            });
        }
        ...
        public void WithoutOffsetPattern()
        {
            TestContext.Run((string target, bool expected) =>
            {
                new Regex(OracleDbQuerySqlBuilder.WithoutOffsetPattern, RegexOptions.IgnoreCase).IsMatch(target).Is(expected);
            });
        }
            ...
            bool xRequestContinuationNeedsTopCount = false)
        {
            ...
            {
                repositoryInfo = new RepositoryInfo(Guid.NewGuid(), repositoryName, new Tuple<string, bool, Guid?>(Guid.NewGuid().ToString(), false, Guid.Parse(_physicalRepositoryId)));
            }
