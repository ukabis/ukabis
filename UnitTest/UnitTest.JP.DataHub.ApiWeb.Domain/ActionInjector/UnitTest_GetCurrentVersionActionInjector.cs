...
{
    [TestClass]
    public class UnitTest_GetCurrentVersionActionInjector : UnitTestBase
    {
        ...
        public void Execute_リポジトリ1つ()
        {
            ...
            var target = UnityCore.Resolve<GetCurrentVersionActionInjector>();
            target.Target = action;
            target.Execute(() => { });

            var result = target.ReturnValue as HttpResponseMessage;
            result.StatusCode.Is(HttpStatusCode.OK);
            result.Content.ReadAsStringAsync().Result.Is(JToken.FromObject($"{{ \"CurrentVersion\" : 1 }}").ToString());

            mockResourceVersionRepository.Verify(x => x.GetCurrentVersion(It.IsAny<RepositoryKey>()), Times.Exactly(1));
        }
        ...
        public void Execute_リポジトリ2つ()
        {
            ...
            mockResourceVersionRepository1.Verify(x => x.GetCurrentVersion(It.IsAny<RepositoryKey>()), Times.Exactly(1));
            mockResourceVersionRepository2.Verify(x => x.GetCurrentVersion(It.IsAny<RepositoryKey>()), Times.Exactly(1));
        }

        [TestMethod]
        public void Execute_リポジトリ2つ_1Null()
        {
            var action = CreateRegistDataAction();
            var mock1 = ActionInjectorCommon.CreateRepositoryMock(UnityContainer, action, null);
            var mockRepository1 = mock1.Item1;
            var mockResourceVersionRepository1 = mock1.Item2;
            ...
            var result = target.ReturnValue as HttpResponseMessage;
            result.StatusCode.Is(HttpStatusCode.OK);
            result.Content.ReadAsStringAsync().Result.Is(JToken.FromObject($"{{ \"CurrentVersion\" : 2 }}").ToString());
