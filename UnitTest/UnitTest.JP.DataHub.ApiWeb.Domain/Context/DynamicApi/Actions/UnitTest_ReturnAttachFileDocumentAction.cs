...
{
    [TestClass]
    public class UnitTest_ReturnAttachFileDocumentAction : UnitTestBase
    {
        [TestInitialize]
        ...
        public void ExecuteAction_正常系()
        {
            ...
            var action = CreateAction(isDocumentHistory: true, versionKey: VersionKeyLatest, documentkey: driveoutedDocumentKey);
            var result = action.ExecuteAction();
        ...
        private readonly string driveoutedDocumentKey = Guid.NewGuid().ToString();
        ...
        private ReturnAttachFileDocumentAction CreateAction(bool isDocumentHistory = true, string versionKey = null, string documentkey = null, string attachFileKey = null)
        {
            ReturnAttachFileDocumentAction action = UnityCore.Resolve<ReturnAttachFileDocumentAction>();
            ...
            action.ActionType = new ActionTypeVO(ActionType.ReturnAttachFileDocument);
            ...
            mockDataRepository.Setup(x => x.RegisterOnce(It.IsAny<RegisterParam>())).Callback<RegisterParam>((regparam) =>
            {
                ...
                regparam.Json[nameof(DynamicApiAttachFileInformation.IsUploaded)].ToString().Is(true.ToString());
            });

            mockDataRepository.Setup(x => x.QueryEnumerable(It.IsAny<QueryParam>())).Returns<QueryParam>((param) =>
            {
                if ((param.QueryString.Dic[new QueryStringKey("FileId")].Value == normalDocumentKey) ||
                (param.QueryString.Dic[new QueryStringKey("FileId")].Value == driveoutedDocumentKey))
                {
                    return new List<JsonDocument>() { new JsonDocument(JToken.Parse(GetInfoJson(attachFileKey, true, documentkey))) };
                }
                else
                {
                    return null;
            ...
            //mockDataRepository.Setup(x => x.QueryEnumerable(It.Is<QueryParam>(y => y.QueryString[new QueryStringKey("FileId")].Value == nouploadversionKey))).Returns((IEnumerable<JsonDocument>)null);
            mockDataRepository.Setup(x => x.QueryOnce(It.IsAny<QueryParam>())).Returns<QueryParam>((param) =>
            {
                if ((param.QueryString.Dic[new QueryStringKey("FileId")].Value == normalDocumentKey) ||
                (param.QueryString.Dic[new QueryStringKey("FileId")].Value == driveoutedDocumentKey))
                {
                    return new JsonDocument(JToken.Parse(GetInfoJson(attachFileKey, true, documentkey)));
                }
                else
                {
                    return null;
            ...
            //mockDataRepository.Setup(x => x.QueryOnce(It.Is<QueryParam>(y => y.QueryString[new QueryStringKey("FileId")].Value == nouploadversionKey))).Returns((JsonDocument)null);
            mockFileRepository.Setup(x => x.GetAttachFileFileStream(It.Is<DynamicApiAttachFileInformation>(_ => _.FileId.ToString() == normalDocumentKey))).Returns(new MemoryStream(System.Text.Encoding.UTF8.GetBytes("hoge")));
            mockFileRepository.Setup(x => x.GetAttachFileFileStream(It.Is<DynamicApiAttachFileInformation>(_ => _.FileId.ToString() == driveoutedDocumentKey))).Throws(new NotFoundException());
            ...
            mockDataRepository.Setup(x => x.DocumentVersionRepository.GetDocumentVersion(It.Is<DocumentKey>(_ => _.Id == normalDocumentKey || _.Id == driveoutedDocumentKey))).Returns(resultNormalHistory);
            ...
            mockDataRepository.Setup(x => x.DocumentVersionRepository.SaveDocumentVersion(It.Is<DocumentKey>(_ => _.Id == normalDocumentKey || _.Id == driveoutedDocumentKey), It.IsAny<RepositoryKeyInfo>(), It.Is<bool>(y => y)))
                .Returns((DocumentKey dockey, RepositoryKeyInfo keyinfo, bool isdelete) =>
                {
                    SavedDocumentHisotry = resultInitialDeleteHistory;
                    return resultInitialDeleteHistory;
                });
            mockDataRepository.Setup(x => x.DocumentVersionRepository.SaveDocumentVersion(It.Is<DocumentKey>(_ => _.Id == normalDocumentKey || _.Id == driveoutedDocumentKey), It.IsAny<RepositoryKeyInfo>(), It.IsAny<DocumentHistory>(), It.IsAny<bool>()))
