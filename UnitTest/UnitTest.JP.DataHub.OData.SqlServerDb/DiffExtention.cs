...
namespace Microsoft.VisualStudio.TestTools.UnitTesting
{
    public static class DiffExtention
    {
        ...
        public static IList<KeyValuePair<T, char>> Diff<T>(this IEnumerable<T> a, IEnumerable<T> b)
        {
        ...
        public static IList<KeyValuePair<string, string>> StringDiffOnly(this string a, string b)
        {
            var result = new List<KeyValuePair<string, string>>();
            var diff = Diff(a, b);
            for (int i = 0; i < diff.Count;)
            {
                int hit = -1;
                for (int j = i; j < diff.Count; j++)
                {
                    if (diff[j].Value != '=')
                    {
                        hit = j;
                ...
                if (hit != -1)
                {
                    ...
                    string right = null;
                    int hit2 = -1;
                    for (int j = hit; j < diff.Count; j++)
                    {
                        if (diff[j].Value == '-')
                        {
                            left += diff[j].Key.ToString();
                        }
                        else if (diff[j].Value == '+')
                        {
                            right += diff[j].Key.ToString();
                        }
                        else if (diff[j].Value == '=')
                        {
                            hit2 = j;
                    ...
                    i = hit2 == -1 ? diff.Count : hit2;
                    result.Add(new KeyValuePair<string, string>(left, right));
                }
                else
                {
                    break;
        ...
        // ------------------------------------------------------------------------
        // 最小編集距離を求めつつ、ついでに経路情報を記録するよ
        // 昔のアルゴリズムって一つの関数に複数の目的を詰め込んだ処理が多い気がする
        private static IEnumerable<Point> Snake<T>(IEnumerable<T> a, IEnumerable<T> b, ref int[] fp, int k, int offset)
        {
            var index = k + offset;
        ...
        private static IEnumerable<Point> Snake<T>(IEnumerable<T> a, IEnumerable<T> b, int k, int y)
        {
            int x = y - k;
        ...
        // ---------------------------------
        // 結果データを作ります
        private static IList<KeyValuePair<T, char>> MakeResultData<T>(this List<Path> paths, IEnumerable<T> a, IEnumerable<T> b, bool swap)
        {
        ...
        private static Path BuildAddedElementData<T>(this List<Path> paths, Point tail, IEnumerable<T> sequence, bool swap, ref List<KeyValuePair<T, char>> resultSequence)
        {
            return paths.BuildChangedElementData(tail, sequence, swap, ref resultSequence, isAdded: true);
        }

        private static Path BuildDeletedElementData<T>(this List<Path> paths, Point tail, IEnumerable<T> sequence, bool swap, ref List<KeyValuePair<T, char>> resultSequence)
        {
            return paths.BuildChangedElementData(tail, sequence, swap, ref resultSequence, isAdded: false);
        }

        private static Path BuildChangedElementData<T>(this List<Path> paths, Point tail, IEnumerable<T> sequence, bool swap, ref List<KeyValuePair<T, char>> resultSequence, bool isAdded)
        {
        ...
        private static Path BuildCommonElementData<T>(this Path path, IEnumerable<T> sequence, ref List<KeyValuePair<T, char>> resultSequence)
        {
        ...
        // -------------------------------------------
        // 経路情報から余分な枝を刈ります∩
        private static List<Path> Prune(this IEnumerable<Path> paths, bool swap)
        {
            return paths.Prune(paths.Last(), swap).ToList();
        }

        private static IEnumerable<Path> Prune(this IEnumerable<Path> paths, Path last, bool swap)
        {
