...
{
    public class SensorTraceabilityGetAggregateSensorFilter : AbstractApiFilter
    {
        private static readonly string ManagementApi = "/API/Traceability/V3/Public/SensorTraceability/Summary/GetLatest?ProductCode={0}&ArrivalId={1}&Time={2}";
        ...
        public override HttpResponseMessage BeforeAction(IApiFilterActionParam param)
        {
            ...
            var timeStr = param.QueryStringDic.GetOrDefault("Time");
            ...
            if (!int.TryParse(timeStr, out var time) || time <= 0)
            {
                return ErrorCodeMessage.GetRFC7807HttpResponseMessage(ErrorCodeMessage.Code.E103408, param.LanguageInfo, $"{param.ResourceUrl}/{param.ApiUrl}");
            }

            // 管理用データ取得
            var result = param.ApiHelper.ExecuteGetApi(string.Format(ManagementApi, productCode, arrivalId, time))
