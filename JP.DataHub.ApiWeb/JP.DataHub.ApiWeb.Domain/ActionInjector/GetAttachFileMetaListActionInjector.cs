...
{
    internal class GetAttachFileMetaListActionInjector : AbstractAttachFileActionInjector
    {
        public override void Execute(Action action)
        {
            ...
            // ODataによるメタの検索に対応しているリポジトリの場合はODataクエリを指定する
            // 非対応のリポジトリはリポジトリ側に任せる
            if (target.DynamicApiDataStoreRepository[0].CanQueryAttachFileMetaByOData)
            {
                string query = "";
                if (target.Query != null)
                {
                    foreach (var q in target.Query.Dic)
                    {
                        ...
                        query += $"MetaList/any(o: o/MetaKey eq '{q.Key.Value}' and o/MetaValue eq '{q.Value.Value}')";
                    }
                }
                ...
                if (!string.IsNullOrEmpty(query))
                {
                    odataParam.Add(new QueryStringKey("$filter"), new QueryStringValue(query));
                }
            ...
            target.OperationInfo = new OperationInfo(OperationInfo.OperationType.AttachFileMeta, selectFields);
            ...
            {
                var metaList = actionResult.Content.ReadAsStringAsync().Result.ToJson() as JArray;
                ...
                ReturnValue = new HttpResponseMessage(actionResult.StatusCode) { Content = new StringContent(metaList.ToString(), Encoding.UTF8, MEDIATYPE_JSON) };
            }
        }
    }
}
