...
{
    internal class SetNewVersionActionInjector : ActionInjector
    {
        public override void Execute(Action action)
        {
            ...
            var parallelResult = target.DynamicApiDataStoreRepository
                ...
                .Select(repository =>
                {
                    ...
                    try
                    {
                        var result = repository.ResourceVersionRepository.AddNewVersion(target.RepositoryKey);
                        //var result = repository.AddNewVersion(target.RepositoryKey, repository.RepositoryInfo);
                        return result;
                    }
            ...
            var currentVersion = JToken.FromObject($"{{ \"CurrentVersion\" : {parallelResult[0].Value} }}");
            ReturnValue = new HttpResponseMessage(HttpStatusCode.Created) { Content = new StringContent(currentVersion.ToString(), Encoding.UTF8, MediaTypeConst.ApplicationJson) };
        }
    }
}
