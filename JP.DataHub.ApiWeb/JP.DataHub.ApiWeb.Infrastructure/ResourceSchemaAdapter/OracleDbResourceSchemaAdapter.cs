...
{
    // TODO: できればデータソース非依存部分を切り離してドメインとインフラに分離したい。
    //    ドメイン：スキーマバリデーションのIF、変更対象列の抽出、抽象化された列定義
    //    インフラ：スキーマバリデーションの実装、SQL生成、列定義のデータソース固有型への変換
    internal class OracleDbResourceSchemaAdapter : IResourceSchemaAdapter
    {
        /// <summary>
        /// PK列
        /// </summary>
        private static readonly ColumnInfo PrimaryKey =
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.ID, $"nvarchar2({OracleDbConsts.PrimaryKeyTableColumnMaxLength})", false), Index = false };
        ...
        private static readonly List<ColumnInfo> ManagementColumns = new List<ColumnInfo>()
        {
            ...
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.VENDORID, "char(36)", true), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.SYSTEMID, "char(36)", true), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.OWNERID, "char(36)", true), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.OPENID, "char(36)", true), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.REGDATE, "timestamp", false), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.UPDUSERID, "char(36)", true), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.UPDDATE, "timestamp", false), Index = true },
            new ColumnInfo() { Column = new RdbmsTableColumn(JsonPropertyConst.ETAG, "int", false), Index = false }
        };

        private readonly JPDataHubLogger Log = new JPDataHubLogger(typeof(OracleDbResourceSchemaAdapter));
        ...
        public OracleDbResourceSchemaAdapter(RepositoryInfo repositoryInfo, ControllerId controllerId, DataSchema controllerSchema)
        {
        ...
        public bool IsAdaptable(out RFC7807ProblemDetailExtendErrors problemDetail)
        {
            ...
            var modifiedColumns = TableColumns.Where(x =>
                ...
                !ControllerSchema.Properties.Any(y => x.Name == y.Key && x.DataType.ToUpper() == y.Value.GetOracleDbDataType(true).ToUpper())).ToList();
        ...
        private void CreateTable(string tableName, JSchema resourceSchema)
        {
            ...
            columns.AddRange(resourceSchema.Properties.Select(x => new RdbmsTableColumn(x.Key, x.Value.GetOracleDbDataType(), true)));
            ...
            // CREATE INDEX(NCLOB)はインデックス作成不可のため除外)
            var indexingColumns = ManagementColumns.Where(x => x.Index).Select(x => x.Column.Name).ToList();
            indexingColumns.AddRange(resourceSchema.Properties.Where(x =>
                !x.Value.GetOracleDbDataType().Contains("MAX") &&
                !x.Value.GetOracleDbDataType().Contains("NCLOB")).Select(x => x.Key).ToList());
        ...
        private void AlterTable(string tableName, List<RdbmsTableColumn> tableColumns, JSchema resourceSchema)
        {
            ...
            var additionalColumns = resourceSchema.Properties
                .Where(x =>
                    ...
                    !tableColumns.Any(y => x.Key == y.Name && x.Value.GetOracleDbDataType(true).ToLower() == y.DataType.ToLower()))
                .Select(x => new RdbmsTableColumn(x.Key, x.Value.GetOracleDbDataType(), true)).ToList();
            ...
            // CREATE INDEX(nvarchar(MAX)はインデックス作成不可のため除外)
            // CREATE INDEX(NCLOB)はインデックス作成不可のため除外)
            var indexingColumns = additionalColumns.Where(x => !x.DataType.Contains("MAX")&& !x.DataType.Contains("NCLOB")).Select(x => x.Name).ToList();
            indexingColumns.ForEach(x => DataStoreRepository.CreateIndex(tableName, x));
        }
