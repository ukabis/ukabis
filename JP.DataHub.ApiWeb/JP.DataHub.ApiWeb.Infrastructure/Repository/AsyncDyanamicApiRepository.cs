...
{
    // .NET6
    internal class AsyncDyanamicApiRepository : AbstractRepository, IAsyncDyanamicApiRepository
    {
        #region CACHEKEY
        ...
        private Lazy<IBlobStorage> _lazyBlobStorage = new Lazy<IBlobStorage>(() => UnityCore.Resolve<IBlobStorage>("AsyncDynamicApiBlobStorage"));
        private IBlobStorage _blobStorage { get => _lazyBlobStorage.Value; }
        #endregion
        ...
        public HttpResponseMessage GetResult(AsyncRequestId asyncRequestId)
        {
            ...
            try
            {
                CloudAppendBlob bodyBlob;
                CloudAppendBlob headBlob;
                if (string.IsNullOrEmpty(requestContinuation))
                {
                    bodyBlob = _blobStorage.GetAppendBlobAsync(container, path + "/Body.bin").Result;
                    headBlob = _blobStorage.GetAppendBlobAsync(container, path + "/Header.bin").Result;
                }
                else
                {
                    var requestContinuationPath = HashCalculation.ComputeHashString(Encoding.Unicode.GetBytes(requestContinuation));
                    bodyBlob = _blobStorage.GetAppendBlobAsync(container, path + "/" + requestContinuationPath + "/Body.bin").Result;
                    headBlob = _blobStorage.GetAppendBlobAsync(container, path + "/" + requestContinuationPath + "/Header.bin").Result;
                }
                var content = new StreamContent(bodyBlob.OpenReadAsync().Result);
                var headerJson = headBlob.DownloadTextAsync().Result?.ToJson();
                //旧形式のヘッダ出来た場合は詰め替える
        ...
        {
            var container = blobPath.Split('/')[0];
            var blobName = string.Join("/", blobPath.Split('/').Skip(1));
            _ = _blobStorage.AppendBlobAsync(container, blobName, content, accept).Result;
        ...
        public bool SetResultOverwrite(Stream content, string blobPath, string accept)
        {
            var container = blobPath.Split('/')[0];
            var blobName = string.Join("/", blobPath.Split('/').Skip(1));
            _ = _blobStorage.OverwriteAppendBlobAsync(container, blobName, content, accept).Result;
