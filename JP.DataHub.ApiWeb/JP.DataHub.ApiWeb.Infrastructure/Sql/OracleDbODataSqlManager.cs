...
using JP.DataHub.OData.OracleDb.ODataToSqlTranslator;
...
namespace JP.DataHub.ApiWeb.Infrastructure.Sql
{
    ...
    /// OracleDbのOData変換
    /// </summary>
    internal class OracleDbODataSqlManager : ODataSqlManager, IODataSqlManager
    {
        ...
        public override string CreateSqlQuery(QueryParam queryParam, string queryString, string sqlWhere2, int sqlMode, out int? topCount, out int? skipCount, out Dictionary<string, object> parameters)
        {
            ...
            try
            {
                //var uri = new Uri("https://localhost/?" + (queryString ?? queryString.Replace("%22", "")));
                var uri = new Uri("https://localhost/?" + (String.IsNullOrEmpty(queryString) ? "" : queryString.Replace("%22", "")));
        ...
        private SQLQueryFormatter SetupSQLQueryFormatter(QueryParam queryParam)
        {
            ...
            if (queryParam?.ControllerSchema?.Value != null)
            {
                ...
                validator = new OracleDbODataFilterValidator(schema);
            }
