...
{
    internal class OracleDbQuerySqlBuilder : IQuerySqlBuilder
    {
        ...
        public OracleDbQuerySqlBuilder(
        ...
        public void BuildUp()
        {
            ...
            else
            {
                ...
                if (QueryParam.IsNative == true && QueryParam.NativeQuery?.IsAllowAddCondition == false)
                {
                    ...
                    parameters = new OracleDbSqlParameterList();
            ...
            Sql = querySql;
            CountSql = isNativeCountQuery ? querySql : $"SELECT COUNT(1) Count FROM ({AppendOrderByOffset(baseSql)}) x"; ;
        ...
        private string CreateDefaultQuery(QueryParam param)
        {
            ...
            return $"SELECT {string.Join(",", selectFields.Select(x => $"\"{x}\""))} FROM {{{OracleDbConsts.TableNameVariable}}}"; // WITH(NOLOCK)
        }
        ...
        private string ReplaceTableName(string sql, QueryParam param)
        {
            ...
            var pattern = $"{{{OracleDbConsts.TableNameVariable}(:[-_0-9a-zA-Z]+)?}}";
        ...
        {
            var physicalRepositoryId = DynamicApiRepository.GetPhysicalRepositoryIdByControllerId(controllerId, RepositoryType.OracleDb);
        ...
        private (string, IRdbmsSqlParameterList) GenerateAttachFileMetaQuerySqlAndParameters(QueryParam param)
        {
            ...
            // SQL組み立て
            var where = CreateWheheString(parameters);
            var sql = $"SELECT {param.OperationInfo.AttachFileOperation.QuerySelectFields} FROM \"{SqlServerConsts.AttachFileMetaTableName}\" afm {where}";
            ...
            foreach (var q in param.QueryString.Dic)
            {
                ...
                sql += $" AND EXISTS(SELECT 1 FROM \"{SqlServerConsts.AttachFileMetaSearchTableName}\" afs WHERE afm.\"id\" = afs.\"AttachFileMetaId\" AND afs.\"MetaKey\" = :MetaKey{i} AND afs.\"MetaValue\" = :MetaValue{i})";
