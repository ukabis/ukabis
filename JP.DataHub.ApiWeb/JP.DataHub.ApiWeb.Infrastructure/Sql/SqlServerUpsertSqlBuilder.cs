...
{
    internal class SqlServerUpsertSqlBuilder : IUpsertSqlBuilder
    {
        #region SQL

        private const string SqlAttachFileMetaSearch = @$"
INSERT INTO {SqlServerConsts.AttachFileMetaSearchTableName}
(
    id,
    AttachFileMetaId,
    MetaKey,
...
SELECT
    ...
    _Upduser_Id,
    _Upddate
FROM {SqlServerConsts.AttachFileMetaTableName} CROSS APPLY OPENJSON(MetaList) WITH
(
...
WHERE FileId = @FileId";
        ...
        public SqlServerUpsertSqlBuilder(
        ...
        private (string, IRdbmsSqlParameterList) GenerateRegisterSqlAndParameters(RegisterParam param, string tableName)
        {
                ...
                param.OperationInfo?.IsVersionOperation == true)
            {
                ...
                sql.Append($"AND \"{tableName}\".\"{JsonPropertyConst.ETAG}\" = {parameters[etagCurrent].SqlParameter} ");
            }
            sql.AppendLine($"THEN");
        ...
        private IRdbmsSqlParameterList PrepareRegisterParameters(RegisterParam param)
        {
            ...
            {
                case OperationInfo.OperationType.VersionInfo:
                    schema = SqlServerVersionInfo.VersionTableSchema;
                    break;
                case OperationInfo.OperationType.DocumentVersion:
                    schema = SqlServerDocumentVersion.DocumentVersionTableSchema;
                    break;
                default:
            ...
            if (param.OperationInfo?.IsVersionOperation == true)
            {
                ...
                parameters.Add(SqlServerVersionInfo.VersionInfoColumnName, versionInfo.ToString());
            }
