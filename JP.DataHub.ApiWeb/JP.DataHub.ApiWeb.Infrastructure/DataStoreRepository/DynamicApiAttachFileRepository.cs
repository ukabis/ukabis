...
{
    // .NET6
    internal class DynamicApiAttachFileRepository : NewBlobDataStoreRepository, IDynamicApiAttachFileRepository
    {
        private static readonly JPDataHubLogger logger = new JPDataHubLogger(typeof(DynamicApiAttachFileRepository));
        ...
        {
            DefaultContainerFormat = (Dictionary<string, string> dic, JToken json, string defaultContainerName) => GetRelativePathToBlobContainerName(dic[nameof(RegisterParam.FilePath)]);
            DefaultFileNameFormat = (Dictionary<string, string> dic, JToken json, string defaultContainerName) => GetRelativePathToBlobName(dic[nameof(RegisterParam.FilePath)]);
        }
