...
{
    // .NET6
    internal class NewBlobDataStoreRepository : NewAbstractDynamicApiDataStoreRepository, INewBlobDataStoreRepository
    {
        ...
        {
            var blobStorage = new BlobStorage(new AzureStorageSetting(ToConnectionString(RepositoryInfo.ConnectionString)));
            string container = ToContainerName(null);
            string filename = ToFileName(null);
            var blob = blobStorage.GetBlockBlobAsync(container, filename).Result;
            using (var s = blob.OpenReadAsync().Result)
            {
                using (var sr = new StreamReader(s))
                {
                    var json = sr.ReadToEnd().ToJson();
        ...
        public Uri GetUriWithSharedAccessSignature(QueryParam query)
        {
            ...
            string container;
            string filename;
            (container, filename) = GetContainerBlobName(query.FilePath.Value);

            return new Uri(blobStorage.GetBlobOntimeTokenAsync(container, filename, SharedAccessBlobPermissions.Read, expireMinutes: 30).Result);
        }
        ...
        public Stream QueryToStream(QueryParam param)
        {
            try
            {
                ...
                if (param.HasFilePath)
                {
                    ...
                    var blobStorage = new BlobStorage(new AzureStorageSetting(connectionstring));
                    (container, filename) = GetContainerBlobName(param.FilePath.Value);
                    var blob = blobStorage.GetBlockBlobAsync(container, filename).Result;
                    return blob.OpenReadAsync().Result;
                }
                else
                {
                    BlobStorage blobStorage = new BlobStorage(new AzureStorageSetting(ToConnectionString(RepositoryInfo.ConnectionString)));
        ...
        public override RegisterOnceResult RegisterOnce(RegisterParam param)
        {
            ...
            var blobStorage = new BlobStorage(new AzureStorageSetting(RepositoryInfo.ConnectionString));
            ...
            if (param.HasStream)
            {
                ...
                _ = blobStorage.SaveBlobAsync(container, filename, param.Stream).Result;
            }
            else if (param.SourceUri != null)
            {
                _ = blobStorage.CopyBlob(param.SourceUri, container, filename).Result;
            }
            else
            {
                ...
                using (var memoryStream = new MemoryStream(byteArray))
                {
                    ...
                    _ = blobStorage.SaveBlobAsync(container, filename, memoryStream).Result;
                }
            }
        ...
        public override void DeleteOnce(DeleteParam param)
        {
            try
            {
                ...
                if (param.HasFilePath)
                {
                    ...
                    (containerName, blobName) = GetContainerBlobName(param.FilePath.Value);
                }
                else
                {
                    containerName = ToContainerName(null);
                    blobName = ToFileName(null);
                }

                BlobStorage blobStorage = new BlobStorage(new AzureStorageSetting(connectionString));

                var blob = blobStorage.GetBlockBlobAsync(containerName, blobName).Result;
                ...
                {
                    var tmp = blob.DeleteIfExistsAsync().Result;
                }
            }
            catch (StorageException se)
            {
        ...
        public override IEnumerable<string> Delete(DeleteParam param)
        {
            ...
            {
                BlobStorage blobStorage = new BlobStorage(new AzureStorageSetting(ToConnectionString(RepositoryInfo.ConnectionString)));
                var list = blobStorage.GetBlobListAsync(param.VendorId.Value, param.FilePath.Value).Result;

                if (param.ThrowNotFoundExcption)
                {
                    list.ForEach(x => x.DeleteAsync());
                }
                else
                {
                    list.ForEach(x => x.DeleteIfExistsAsync());
                }
                return list.Select(x => x.Name.ToString());
            }
            catch (StorageException se)
            {
                if (se.RequestInformation.HttpStatusCode == (int)System.Net.HttpStatusCode.NotFound)
        ...
        public Uri CopyFile(string destBlobName, string destContainerName, string srcBlobName, string srcContainerName)
        {
            BlobStorage blobStorage = new BlobStorage(new AzureStorageSetting(ToConnectionString(RepositoryInfo.ConnectionString)));
            return blobStorage.CopyBlobAsync(srcContainerName, srcBlobName, destContainerName ?? srcContainerName, destBlobName).Result;
        }

        public Uri CopyFile(string destBlobName, string destContainerName, Uri srcUri)
        {
            BlobStorage blobStorage = new BlobStorage(new AzureStorageSetting(ToConnectionString(RepositoryInfo.ConnectionString)));
            return blobStorage.CopyBlob(srcUri, destContainerName, destBlobName).Result;
        }
...
", new { RepositoryGroupId = repository_group_id, PhysicalRepositoryId = physical_repository_id }).FirstOrDefault();
        ...
        private (string, string) GetContainerBlobName(string blobPath)
        {
            string endpoint = GetFilePathToEndpoint(blobPath);
            var tmpPath = blobPath.Substring(endpoint.Length);
            var split = tmpPath.Split('/');
            var containerName = split[0];
            var blobName = tmpPath.Substring(split[0].Length + 1);
        ...
        protected string GetRelativePathToBlobContainerName(string filePath)
        {
        ...
        protected string GetRelativePathToBlobName(string filePath)
        {
