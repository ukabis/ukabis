...
namespace JP.DataHub.ApiWeb.Infrastructure.DataStoreRepository
{
    ...
    /// </summary>
    internal class SqlServerDataStoreRepository : NewAbstractDynamicApiDataStoreRepository, INewDynamicApiDataStoreRdbmsRepository
    {
        private static readonly JPDataHubLogger s_log = new JPDataHubLogger(typeof(SqlServerDataStoreRepository));

        private Lazy<IConfigurationSection> _lasyConfiguration = new Lazy<IConfigurationSection>(() => UnityCore.Resolve<IConfiguration>().GetSection("SqlServerDataStoreRepository"));
        ...
        public override string VersionInfoQuery { get => $"SELECT {SqlServerVersionInfo.VersionInfoColumnName}, {JsonPropertyConst.ETAG} FROM {SqlServerVersionInfo.VersionTableName} WITH(NOLOCK) WHERE id = {ToSqlParameter("id")}"; }
        public override string DocumentVersionQuery { get => $"SELECT * FROM {SqlServerDocumentVersion.DocumentVersionTableName} WITH(NOLOCK) WHERE id = {ToSqlParameter("id")}"; }
        public override string RepositoryName => "SqlServer";
        ...
        /// マルチバイト文字列の最大文字数
        /// </summary>
        public int? NCharTableColumnMaxLength => SqlServerConsts.NCharTableColumnMaxLength;

        /// <summary>
        /// テーブル列名の最大バイト数
        /// </summary>
        public int? TableColumnNameMaxBytes => SqlServerConsts.TableColumnNameMaxLength;

        /// <summary>
        /// テーブル最大列数
        /// </summary>
        public int? TableMaxColumns => SqlServerConsts.TableMaxColumns;

        /// <summary>
        /// テーブル列名パターン
        /// </summary>
        public string TableColumnNamePattern => SqlServerConsts.TableColumnNamePattern;
        ...
        /// <summary>
        /// SqlServerへの操作リトライ秒数(type:double)。指数関数的バックオフで秒を指定しているので秒数=待ち時間ではない
        /// </summary>
        ...
        /// インスタンスを初期化します。
        /// </summary>
        public SqlServerDataStoreRepository()
        {
        ...
        /// <summary>
        /// SQLServerへのコネクションを取得する。同時に接続のオープンも行う。
        /// </summary>
        ...
        public override JsonDocument QueryOnce(QueryParam param)
        {
            ...
            if (doc != null && param.OperationInfo?.IsVersionOperation == true)
            {
                ...
                var versionInfo = doc.Item1[SqlServerVersionInfo.VersionInfoColumnName];
            ...
            else
            {
                ...
                switch (param.OperationInfo?.Type)
                {
                    case OperationInfo.OperationType.VersionInfo:
                        propertySchemaList = SqlServerVersionInfo.VersionTableSchema.Properties;
                        break;
                    case OperationInfo.OperationType.DocumentVersion:
                        propertySchemaList = SqlServerDocumentVersion.DocumentVersionTableSchema.Properties;
                        break;
                    default:
        ...
        private IEnumerable<JToken> ExecuteQuery(string sql, IRdbmsSqlParameterList sqlParameters, bool isCustomQuery = false)
        {
            ...
            {
                s_log.Warn($"SqlServer SQL Error: {ex.Message}, SQL: {sql}", ex);
        ...
        public override int ODataPatch(QueryParam queryParam, JToken patchData)
        {
            ...
            {
                s_log.Warn($"SqlServer SQL Error: {ex.Message}, SQL: {sqlBuilder.Sql}", ex);
                return 0;
            }
            catch (SqlException ex)
            {
        ...
        public IEnumerable<RdbmsTableColumn> GetTableColumns(string tableName)
        {
            ...
            var sql = $"SELECT name AS COLUMN_NAME, TYPE_NAME(user_type_id) + CASE WHEN max_length = -1 THEN '(MAX)' ELSE '' END AS DATA_TYPE FROM sys.columns WHERE object_id = (SELECT object_id FROM sys.tables WHERE name = {SqlBuilderUtil.ToSqlParameter(tableNameParam, RepositoryInfo.Type)})";
        ...
        /// SQLパラメータ用の文字列に変換
        /// </summary>
        private static string ToSqlParameter(string parameterName) => SqlBuilderUtil.ToSqlParameter(parameterName, RepositoryType.SQLServer2);
    }
}
