...
namespace JP.DataHub.ManageApi.Controllers
{
    ...
    {
        [Dependency]
        public IVendorAuthenticationService VendorAuthenticationService { get; set; }
        ...
        public IActionResult Post([FromForm] TokeRequestModel model)
        {
            ...
            var result = VendorAuthenticationService.AuthenticateClientId(model.client_id, model.client_secret);
            if (result == null)
            {
                return BadRequest(new Dictionary<string, string> { { "erros", "client_id,client_secret" } });
            }
