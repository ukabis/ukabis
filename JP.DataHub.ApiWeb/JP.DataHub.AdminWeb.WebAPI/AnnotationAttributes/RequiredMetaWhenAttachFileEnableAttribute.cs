...
namespace JP.DataHub.AdminWeb.WebAPI.AnnotationAttributes
{
    ...
    public class RequiredMetaWhenAttachFileEnableAttribute : RequiredAttribute
    {
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (string.IsNullOrEmpty(value as string))
            {
                if (model?.IsEnable == true)
                    return new ValidationResult("添付ファイルを有効にする場合はMetaRepositoryを設定してください。", new List<string>() { validationContext.DisplayName });
            }
